...
- [x] D1 Connect one-function bridge consumption to Issue 097 routing while enumerating complete English/Korean lifecycle aliases and Git operations, splitting only strong sequence/subordinate clauses and binding each operation/action to its nearest semantic target across punctuation and arbitrary modifiers without a bounded filler regex, preserving domain-target advisory validation, exact prerequisite fallback, default-unavailable behavior, and all implementation/review/release ownership boundaries. [files: skills/spec-kit-validation-bridge/SKILL.md, adapters/capability-routing.json, commands/moduflow.md, skills/index/SKILL.md, skills/pm-execution-router/SKILL.md, scripts/spec_kit_adapter.py, tests/test_capability_routing.py, tests/test_spec_kit_adapter.py, tests/test_validation_distribution.py] [depends: C1] [shared_state: true]
