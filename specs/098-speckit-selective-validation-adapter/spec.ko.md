...
- 프로젝트에서 한 번 opt-in한 뒤 `스펙킷 요구사항 분석`처럼 명시 문법으로 요청한다.
- 자유문장 대신 보장된 네 가지 요청 형태만 기억하면 되고 `.specify` 구조는 필요 없다.
...
- 영어는 `접두어 → 함수 → 선택 대상`, 한국어는 `접두어 → 선택 대상 → 함수` 순서만
  허용한다. 알 수 없는 단어, 복수 함수, 추가 행동 문장은 config나 템플릿을 읽기 전에
  fallback과 표준 재시도 예시를 반환한다.

## 검증과 파일럿

영어·한국어 네 함수 성공 8건, 비활성/미설치 4건, 미지원 단어·복수 함수·구두점·구현·
라이프사이클·Git·리뷰·릴리즈·혼합 의도 fallback 12건을 실제 라우터와 어댑터로 검증한다.
...
- opt-in 프로젝트는 문서화된 영어·한국어 명시 문법으로 네 기능을 각각 요청할 수 있다.
- 요청당 템플릿 하나만 로드된다.
...
- 성공 8건, 가용성 fallback 4건, 문법 fallback 12건이 통과한다.
