...
- 정상 설치된 플러그인에는 개발용 Git 이력·전체 테스트·프로젝트 문서가 원래 없습니다. 그런데 원본 저장소나 프로젝트용 검사로 이를 검사해, 정상 패키지도 실패로 판정했습니다.
- 원본 저장소의 버전, 설치 폴더의 버전, 현재 실행 프로세스가 사용하는 버전을 상태·Doctor 출력만으로 확실히 구분하기 어려웠습니다. 따라서 원본 테스트 통과가 현재 채팅에 새 코드가 적용됐다는 증거가 되지 못했습니다.
- 근거: 이슈 102 배포 후 검증에서 접수·확정한 F-003(검사 대상 혼동), F-004(실행 근거 누락). 새 대시보드 요구와는 별개의 기존 결함입니다.

### 사용자에게 달라지는 점

- 설치 상태를 확인할 때 해당 대상에 맞는 검사 결과를 받습니다. 원본에만 있어야 하는 파일이 없다는 이유로 정상 설치를 고장이라고 하지 않습니다.
- 상태를 볼 때 실행 패키지의 버전·위치·출처를 확인해, 코드 문제와 설치/실행 버전 차이를 구분할 근거를 얻습니다. 확인할 수 없는 호스트·스킬 로드 정보는 미확인으로 남깁니다.
- 배포 판단에서 원본 검증, 설치 패키지 검증, 실제 실행 관찰을 구분합니다. 불필요한 재설치·재검사와 잘못된 적용 완료 보고를 줄이는 것이 기대 효과입니다.

### 이번 작업이 아닌 것

대시보드·위키·자동화 실행기 추가, 현재 채팅 자동 갱신, 모든 Claude/Codex 호스트의 로드 상태 보장, 실제 플러그인 배포·설치는 아닙니다. **기능을 더 얹기 위한 확장이 아니라 기존 플러그인을 정확히 진단하기 위한 정비**입니다.
