...
- [ ] A1 Lock legacy Markdown parity, sanitized BIZ fixtures, and the safe top-level YAML subset.
- [ ] A2 Normalize versioned `0.1.0`, unversioned, unsupported, and projection-conflict inputs to `moduflow.issue.v2`.

## Stream B — Structural Readiness and Migration Report

- [ ] B1 Enforce schema, state projection, dependency, definition-readiness, artifact, gate, and next-command precedence.
- [ ] B2 Add `moduflow.issue-migration-report.v1` report-only CLI with before/after routing impact and zero writes.

## Stream C — Consumer Convergence

- [ ] C1 Move lifecycle state, priority, `depends_on`/`Blocked-by`, ready queue, dangling, cycle, and active-unmet drift to the shared normalizer.
- [ ] C2 Put the structural gate before Issue 077 in loop routing, validation, and doctor recommendations.
- [ ] C3 Move MCP and the current dashboard graph/Issue DB data model to normalized state, readiness, diagnostics, and recommended command.

## Stream D — Authoring Compatibility and Distribution

- [ ] D1 Keep generated/template issues canonical Markdown, ship the new module/fixtures, and prevent independent consumer parsers from returning.

## Stream E — Convergence, Dogfood, and Completion Evidence

- [ ] E1 Prove cross-consumer parity for legacy Markdown and BIZ-033/038/039/040, then record focused-test and migration-report evidence.
