# Tasks: Reduce Approval Popup Friction

- [ ] Map approval-triggering commands and classify risk [files: specs/027-reduce-approval-popup-friction/plan.md, docs/*]
- [ ] Normalize importable validation APIs [files: scripts/validate_project_artifacts.py, scripts/validate_moduflow.py, scripts/project_doctor.py, scripts/release_check.py]
- [ ] Keep CLI wrappers backward-compatible [files: scripts/*]
- [ ] Add host/tool adapter guidance for shell-free validation [files: docs/*, specs/027-reduce-approval-popup-friction/status.md]
- [ ] Add focused tests for importable validation paths [files: tests/*]
- [ ] Run validation and release checks [files: specs/027-reduce-approval-popup-friction/status.md]

## Next Command

`/product:execute 027-reduce-approval-popup-friction`
