...
**승인: 이동원, 2026-09-05.** 이 절은 C5에 따라 추가만 합니다. 위의 내용은 하나도 지우거나 번호를 바꾸거나 고쳐 쓰지 않았습니다. 원문과 여기 기록한 유예 사항을 함께 읽어야 합니다.

### 086이 그대로 전달하는 것

단일 프로젝트 대시보드 전체입니다. 기존 DB / 이슈 / 메모리 탭, 새로 추가하는 제작기록 / 플레이북 탭, 전역 프로젝트 선택기 하나, 그리고 등록된 프로젝트 하나로 해석되는 `?project=<id>` URL 상태가 여기에 들어갑니다. 목표 1·3·4·5·6·7·9는 그대로이고, 수용 기준 1·3·4·5·6·8·9·10·11·12·13·14·15·16도 그대로입니다.

### 118로 미루는 것

취소가 아니라 `118-portfolio-mode-dashboard`로 넘기는 범위입니다.

- **목표 2의 포트폴리오 부분** — 등록된 포트폴리오 프로젝트를 선택기 목록의 출처로 재사용하는 부분만 미룹니다. 목표 2의 나머지 절반인 "단일 프로젝트 무설정 사용 유지"는 086에 남습니다. 086이 유지하는 선택기의 전제 조건이기 때문입니다.
- **목표 8** — `전체 프로젝트` 포트폴리오 요약.
- **제안 솔루션 → 두 생성 모드** 중 *Portfolio mode* 갈래 — `projects.json`을 읽고 등록 프로젝트별 제한된 스냅샷을 수집하는 경로. *Project mode*는 그대로입니다. 흐름도에서는 `Registry` → `Resolve` → `Collect` 경로와 `전체 프로젝트` → `요약` → `선택` 경로가 118로 넘어가고, `Local` → `Collect` → `Payload` → `Store` 경로는 남습니다.
- **전역 프로젝트 상태**의 마지막 문단만 — `selectedProjectId=all`을 별도 요약 상태로 두고 상세 링크를 비활성화하는 부분. 원자적 전환 계약 자체는 그대로 남습니다.
- **전체 프로젝트 요약** 절 전체.
- **수용 기준 2** — 포트폴리오 모드가 `moduflow.projects.v1`의 유효 프로젝트와 잘못된 항목 경고를 렌더링한다는 기준.
- **수용 기준 7** — `전체 프로젝트`가 요약만 보여주고 상세/작업 전 특정 프로젝트를 요구한다는 기준.
- URL 계약(100행)과 "프로젝트 간 링크" 리스크(160행)가 요구하는 **신뢰된 단일 링크 resolver**. 지금 존재하지 않으며, payload 하나가 둘 이상의 프로젝트 루트를 가리킬 수 있을 때에야 실제로 필요한 장치입니다.

프로젝트 payload 계약(81~85행)은 문구 그대로 둡니다. 이 개정에 따라 086 산출물의 `PROJECT_DATA.projects`에는 항상 프로젝트가 하나만 들어가지만, 자료 구조를 좁히지는 않았으므로 118은 스키마 변경 없이 여러 개를 채울 수 있습니다.

### 판단 근거

1. **승인된 계획이 이미 제외해 놓고 스스로 알아차리지 못했습니다.** `plan.md:24`는 전역 제약으로 "두 프로젝트의 기록을 한 payload에 섞는 화면은 만들지 않는다"고 못박습니다. 계획의 계약 표에는 단일 프로젝트 진입점(`_collect_production_records(root, ...)`, `_collect_playbooks(root, ...)`, `render_project_view(root, ...)`)만 있고, 파일 맵에는 포트폴리오 수집기가 아예 없습니다. 그런데 같은 계획의 커버리지 표는 AC3~AC7을 C1/C2에 배정해, 수집기가 없는 요약 화면까지 덮인 것처럼 적어 놓았습니다. 계획 문서가 그 자체로 모순입니다. 이 개정은 제약이 이미 가리키고 있던 방향으로 그 모순을 정리합니다.
2. **이후의 거버넌스 결정이 이것을 레이아웃 문제가 아니라 프라이버시 문제로 바꿨습니다.** `workspace/roadmap.md`의 "086/092 — lightweight projections" 항목은 "숨긴 탭은 프라이버시 경계가 아니다. 모든 프로젝트의 비공개 기록을 포트폴리오 HTML payload 하나에 묶지 말 것"이라고 적고 있습니다. 이 결정은 본 스펙보다 나중입니다. 수용 기준 7은 UI 절제로 쓰였지만, 로드맵은 payload 자체를 경계로 봅니다. 이를 지키려면 렌더링 규칙이 아니라 수집 설계가 필요하고, 그 설계는 이 스펙에 없습니다.
3. **스펙이 계획에 넘긴 질문에 계획이 답하지 않았습니다.** "계획 결정" 항목(165행)은 프로젝트별 payload 제한과 상세 데이터를 eager embed할지 프로젝트별 파일로 연결할지 정하라고 요구했지만, 계획은 제한도 정하지 않았고 생성 방식도 고르지 않았습니다.
4. **092는 맡을 자리가 아닙니다.** `issues/092-project-home-dashboard.md:47`은 "명시적 프로젝트 선택 없는 프로젝트 간 상세 화면"을 비범위로 적어 두었습니다. 004와 036은 완료 상태입니다. 맡을 이슈가 없으므로 새 이슈가 필요합니다.
5. **그동안 기능이 사라지지는 않습니다.** `scripts/project_portfolio.py:262`의 `write_dashboard()`가 이미 `portfolio-dashboard.md`로 Markdown 포트폴리오 요약을 만듭니다. 포트폴리오 관점 자체는 계속 있고, HTML 형태만 미뤄집니다.

### 확정: "계획 결정" 항목의 답

165행에 열려 있던 질문을 다음 계획으로 다시 넘기지 않고 여기서 확정합니다.

**포트폴리오 생성은 프로젝트별 상세 파일로 링크한다. 상세를 eager embed하지 않는다.** 기록 본문, 내부 보고 문장, 결정, 메모리 내용을 담는 산출물은 프로젝트 루트별로 생성되는 단일 프로젝트 대시보드뿐입니다. 포트폴리오 산출물은 집계 수치, 주의 상태, 링크만 담고 프로젝트 본문은 담지 않습니다. 따라서 payload 제한은 payload 계약에 이미 적힌 대로 프로젝트 단위로 적용하면 되고, 프로젝트 간 본문 payload 자체가 생기지 않으므로 별도의 통합 제한은 필요 없습니다.

이렇게 하면 로드맵의 제약이 렌더링 규율이 아니라 구조가 됩니다. 모든 프로젝트의 비공개 기록을 묶을 수 있는 payload가 애초에 없으므로, 어떤 화면도 "보여주지 않겠다"는 약속으로 신뢰받을 필요가 없습니다.

### 새 담당 이슈

`118-portfolio-mode-dashboard`가 미룬 범위를 맡습니다. `projects.json` 기반 포트폴리오 수집, `전체 프로젝트` 요약 화면, `all` 선택 상태, 신뢰된 프로젝트 간 링크 resolver가 대상이며 `blocked_by: 086`입니다.
