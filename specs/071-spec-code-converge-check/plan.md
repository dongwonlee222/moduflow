...
6. **CV line grammar** (fixed): `- [ ] CV-<n> [<severity>] <finding> — <source-ref>, from converge <date>` where source-ref is `AC#<k>` or `GC#<k>` (plan Global Constraint). Dedup key = normalized finding text + source-ref; re-runs never re-append a finding whose CV item is still unchecked.
