...
Status: in_progress — transaction engine, public adapters, and D1a Doctor recovery diagnostics are complete; D1b mutator-bypass audit is active
...
- [ ] **D1b** Close transaction/public-writer audit gaps and require a zero-gap operation inventory. | Files: scripts/project_operation_audit.py, config/project-operation-entrypoints.json, tests/test_project_operation_audit.py | Depends: D1a
...
`product:execute 103-atomic-lifecycle-state-transaction` — continue with D1b mutator-bypass audit; full discovery and release gates remain deferred to D2.
