...
- [x] Write denial occurs before every lock, journal, staging, temporary, evidence, or canonical side effect.
- [x] Every planned target is canonical, contained, no-follow, fully hashed, deterministically ordered, and validated in the projected state.
- [x] Failure/crash injection at every journal boundary and target position proves unchanged state, exact rollback, or explicit `recovery_required`.
- [x] Concurrent edits are never overwritten; identical retries return `noop`; key/intent collisions return deterministic conflict.
- [x] Optional physical issue index stays absent unless selected; roadmap prose is untouched unless a roadmap-owned field changes.
